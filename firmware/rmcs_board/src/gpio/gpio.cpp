...
constexpr uint8_t kBmi088IntGyroPin = 15; // PB15, active-low
constexpr uint8_t kBmi088IntAccelPin = 0; // PY00, active-low

void configure_bmi088_int_gyro() {
    HPM_IOC->PAD[IOC_PAD_PB15].FUNC_CTL = IOC_PB15_FUNC_CTL_GPIO_B_15;
    gpiom_set_pin_controller(HPM_GPIOM, GPIOM_ASSIGN_GPIOB, kBmi088IntGyroPin, gpiom_soc_gpio0);
    gpio_set_pin_input(HPM_GPIO0, GPIO_OE_GPIOB, kBmi088IntGyroPin);

    gpio_config_pin_interrupt(
        HPM_GPIO0, GPIO_DI_GPIOB, kBmi088IntGyroPin, gpio_interrupt_trigger_edge_falling);
    gpio_clear_pin_interrupt_flag(HPM_GPIO0, GPIO_IF_GPIOB, kBmi088IntGyroPin);
    gpio_enable_pin_interrupt(HPM_GPIO0, GPIO_IE_GPIOB, kBmi088IntGyroPin);
...
void configure_bmi088_int_accel() {
    ...
    gpiom_set_pin_controller(HPM_GPIOM, GPIOM_ASSIGN_GPIOY, kBmi088IntAccelPin, gpiom_soc_gpio0);
    gpio_set_pin_input(HPM_GPIO0, GPIO_OE_GPIOY, kBmi088IntAccelPin);

    gpio_config_pin_interrupt(
        HPM_GPIO0, GPIO_DI_GPIOY, kBmi088IntAccelPin, gpio_interrupt_trigger_edge_falling);
    gpio_clear_pin_interrupt_flag(HPM_GPIO0, GPIO_IF_GPIOY, kBmi088IntAccelPin);
    gpio_enable_pin_interrupt(HPM_GPIO0, GPIO_IE_GPIOY, kBmi088IntAccelPin);
